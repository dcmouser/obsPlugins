...
static const char *fade_get_name(void *type_data)
{
	...
	return obs_module_text("FadeDelayedTransition");
}

static void *fade_create(obs_data_t *settings, obs_source_t *source)
{
	struct fade_info *fade;
	char *file = obs_module_file("fade_delayed_transition.effect");
	gs_effect_t *effect;
	...
	if (!effect) {
		blog(LOG_ERROR, "Could not find fade_delayed_transition.effect");
		return NULL;
	}
...
// ATTN: These are just used for the AUDIO fade
// it might be natural to make the audio fade delayed just like the video
// but i think maybe we want to fade it the audio sooner
...
struct obs_source_info fade_delayed_transition = {
	.id = "fade_delayed_transition",
...
{
	return "fadeDelayed transition";
}

bool obs_module_load(void)
{
	obs_register_source(&fade_delayed_transition);
	return true;
}
